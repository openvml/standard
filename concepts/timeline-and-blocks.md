...
OVML does not only describe what content exists. It also describes when that content should begin, how long it should remain active, and how it relates to other content on the timeline.
...
afterPrevious is based on the previous textual block, not necessarily the immediately preceding XML element.

This allows media elements to coexist with dialogue without unintentionally changing the dialogue sequence.
...
Authors are encouraged to specify startMode explicitly when timing is important for readability or interoperability.
...
The Player should use the resolved media duration when the duration is not explicitly defined by the OVML document.
...
The Player may use buffering and preloading strategies to ensure that short TTS fragments can be played without audible gaps.
...
The OVML document describes the intended visual behavior; the Player or renderer determines how it is implemented.
...
The same project should produce equivalent temporal relationships across compatible OpenVML implementations.
...
Therefore, the standard defines the semantic timing contract, while implementations control execution details.
...
The Player does not need to know whether timing was created manually, by an AI Assistant, or by another authoring tool.
...
The timing model does not require a specific playback engine, operating system, media framework, TTS provider, or rendering technology.
...
This allows the same OVML document to be consumed by different OpenVML-compatible applications and runtimes while preserving its intended temporal structure.